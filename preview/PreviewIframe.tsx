...
const ErrorBanner = ({ error, onDismiss, onAIFix }: ErrorBannerProps) => {
  ...
  const getTruncatedError = () => {
    ...
    // If first line is short enough, return it
...
interface PreviewIframeProps {
  ...
  onReload?: () => void;
  onRestart?: () => void;
  onActivateComponentSelector?: () => void;
  ...
  availableRoutes?: Array<{ path: string; label: string }>;
}

// Preview iframe component
export const PreviewIframe = ({ loading }: PreviewIframeProps) => {
  ...
  // Use refs to avoid recreating message listener on every dependency change
  ...
  // Using refs to avoid recreating listener on every render
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      // Validate message origin matches the app URL for security
      const currentAppUrl = appUrlRef.current;
      if (currentAppUrl) {
        try {
          const appOrigin = new URL(currentAppUrl).origin;
          if (event.origin !== appOrigin) {
            console.warn('Message from unexpected origin:', event.origin);
            return;
          }
        } catch (e) {
          console.error('Failed to parse app URL for origin validation:', e);
          return;
        }
      }

      if (event.data?.type === "rax-component-selected") {
      ...
      if (
        ...
        if (payload?.newUrl) {
          updateHistoryFromEvent(type, payload.newUrl);
        }
      }
    };
    ...
    // Only include stable dependencies to prevent listener recreation
    setAppOutput,
    setErrorMessage,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 mt-2 preview-iframe-container">
        <div className="relative h-full preview-rounded-fix">
            <div className="preview-corner-mask-bl" />
            <div className="preview-corner-mask-br" />
            <ErrorBanner
              error={errorMessage}
            ...
            {!appUrl ? (
              ...
              <iframe
                ...
                className="w-full h-full border-none iframe-rounded"
                src={appUrl}
...
// Export navigation controls as a separate component
export const PreviewNavigationHeader = ({
  ...
  return (
    <div className="flex items-center p-2 space-x-2 bg-background">
      {/* Navigation Buttons */}
      ...
      {/* Address Bar with Routes Dropdown */}
          ...
          <DropdownMenuContent className="w-full">
            {availableRoutes.length > 0 ? (
              availableRoutes.map((route) => (
                <DropdownMenuItem
                  key={route.path}
                  onClick={() => onNavigateToRoute(route.path)}
...
function parseComponentSelection(data: any): ComponentSelection | null {
  if (
    !data ||
    data.type !== "rax-component-selected" ||
  ...
  const { id, name, selector, tagName, className, textContent } = data;

  // Check if this is a tagged component (format: "filepath:line:column")
  ...
  // Handle universal selector (untagged elements)
  // For untagged elements, we'll create a meaningful description
  const elementDescription = selector || `${tagName}${className ? `.${className.split(' ')[0]}` : ''}`;
  return {
    id: elementDescription,
    name: `${name}${textContent ? ` ("${textContent.substring(0, 30)}...")` : ''}`,
    relativePath: "DOM Element", // Generic indicator for untagged elements
    lineNumber: 1,
    columnNumber: 1,
  };
}
