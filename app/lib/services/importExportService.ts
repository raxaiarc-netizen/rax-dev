...
const logger = createScopedLogger('ImportExport');

// import { getAllChats, deleteChat } from '~/lib/persistence/chats'; // Removed - using cloud storage now
...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          rax_user_profile: this._safeGetItem('rax_user_profile'),
          rax_settings: this._safeGetItem('rax_settings'),
          rax_profile: this._safeGetItem('rax_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('rax_viewed_features'),
          developer_mode: this._safeGetItem('rax_developer_mode'),
        ...
        ui: {
          // Tab configuration
          rax_tab_configuration: this._safeGetItem('rax_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('rax_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('rax_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          rax_read_logs: this._safeGetItem('rax_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('rax_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('rax_chat_history');
   ...
   */
  private static async _importComprehensiveFormat(data: Record<string, unknown>): Promise<void> {
    ...
    if (data.providers && typeof data.providers === 'object') {
      ...
      if (providers.provider_settings) {
        try {
          this._safeSetItem('provider_settings', providers.provider_settings);
        } catch (err) {
    ...
    if (data.ui && typeof data.ui === 'object') {
      ...
      if (ui.rax_tab_configuration) {
        try {
          this._safeSetItem('rax_tab_configuration', ui.rax_tab_configuration);
        } catch (err) {
      ...
      if (ui.promptId) {
        try {
          this._safeSetItem('promptId', ui.promptId);
        } catch (err) {
    ...
    if (data.connections && typeof data.connections === 'object') {
      ...
      if (connections.netlify_connection) {
        try {
          this._safeSetItem('netlify_connection', connections.netlify_connection);
        } catch (err) {
    ...
    if (data.debug && typeof data.debug === 'object') {
      ...
      const debugLocalStorageKeys = [
        'rax_acknowledged_debug_issues',
        'rax_acknowledged_connection_issue',
        'error_logs',
        'rax_read_logs',
      ];
    ...
    if (data.updates && typeof data.updates === 'object') {
      ...
      if (updates.update_settings) {
        try {
          this._safeSetItem('update_settings', updates.update_settings);
        } catch (err) {
      ...
      if (updates.last_acknowledged_update) {
        try {
          this._safeSetItem('rax_last_acknowledged_version', updates.last_acknowledged_update);
        } catch (err) {
    ...
    if (data.chatSnapshots && typeof data.chatSnapshots === 'object') {
