...
describe('StreamingMessageParser', () => {
  ...
  describe('no artifacts', () => {
    ...
    ])('should correctly parse chunks and strip out rax artifacts (%#)', (input, expected) => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ...
      ['Foo bar <bol', 'Foo bar '],
      ['Foo bar <rax', 'Foo bar '],
      ['Foo bar <raxa', 'Foo bar <raxa'],
      ['Foo bar <raxA', 'Foo bar '],
      ['Foo bar <raxArtifacs></raxArtifact>', 'Foo bar <raxArtifacs></raxArtifact>'],
      ['Before <oltArtfiact>foo</raxArtifact> After', 'Before <oltArtfiact>foo</raxArtifact> After'],
      ['Before <raxArtifactt>foo</raxArtifact> After', 'Before <raxArtifactt>foo</raxArtifact> After'],
    ])('should correctly parse chunks and strip out rax artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <raxArtifact title="Some title" id="artifact_1">foo bar</raxArtifact> Some more text',
        {
      ...
      [
        [
          'Some text before <raxArti',
          'fact',
          ' title="Some title" id="artifact_1" type="bundled" >foo</raxArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        [
          ...
          ' ',
          '>',
          'foo</raxArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <raxArtifact title="Some title" id="artifact_1">foo</raxArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <raxArtifact title="Some title" id="artifact_1"><raxAction type="shell">npm install</raxAction></raxArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <raxArtifact title="Some title" id="artifact_1"><raxAction type="shell">npm install</raxAction><raxAction type="file" filePath="index.js">some content</raxAction></raxArtifact> After',
        {
          output: 'Before  After',
