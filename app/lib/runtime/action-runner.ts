...
import type { ActionAlert, RaxAction, DeployAlert, FileHistory, SupabaseAction, SupabaseAlert } from '~/types/actions';
...
export type BaseActionState = RaxAction & {
...
export type FailedActionState = RaxAction &
...
export class ActionRunner {
  ...
  onSwitchView?: (view: 'code' | 'preview' | 'diff') => void;
  buildOutput?: { path: string; exitCode: number; output: string };

  constructor(
    webcontainerPromise: Promise<WebContainer>,
    getShellTerminal: () => RaxShell,
    onAlert?: (alert: ActionAlert) => void,
    onSupabaseAlert?: (alert: SupabaseAlert) => void,
    onDeployAlert?: (alert: DeployAlert) => void,
    onToggleTerminal?: (show: boolean) => void,
  ...
  async #runStartAction(action: ActionState) {
    ...
    // Show terminal when starting the dev server
    if (this.onToggleTerminal) {
      this.onToggleTerminal(true);
    }
    ...
    try {
      ...
      try {
        ...
        needsInstall = true;
        logger.debug('node_modules not found, will run npm install');
      }
    } catch {
      // package.json doesn't exist, no need to install
      logger.debug('No package.json found, skipping install');
    }

    // If we need to install dependencies, do it first
    if (needsInstall && hasPackageJson) {
      logger.info('Running npm install before starting dev server...');

      // Run npm install
      const installResp = await shell.executeCommand(this.runnerId.get(), 'npm install', () => {
        logger.debug('[install]:Aborting Action\n\n');
        action.abort();
      });

      logger.debug(`npm install Response: [exit code:${installResp?.exitCode}]`);
    ...
    // After dev server starts successfully, switch to preview view
    // Add a small delay to let the server fully initialize
    setTimeout(() => {
      if (this.onSwitchView) {
        this.onSwitchView('preview');
        logger.info('Switched to preview view after dev server started');
      }
    }, 2000);
  ...
  async #runBuildAction(action: ActionState) {
    ...
    // Use pnpm for faster build performance
