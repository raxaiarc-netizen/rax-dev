import type { ActionType, RaxAction, RaxActionData, FileAction, ShellAction, SupabaseAction } from '~/types/actions';
import type { RaxArtifactData } from '~/types/artifact';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';

const ARTIFACT_TAG_OPEN = '<raxArtifact';
const ARTIFACT_TAG_CLOSE = '</raxArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<raxAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</raxAction>';
const RAX_QUICK_ACTIONS_OPEN = '<rax-quick-actions>';
const RAX_QUICK_ACTIONS_CLOSE = '</rax-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends RaxArtifactData {
  messageId: string;
  artifactId?: string;
}
...
interface MessageState {
  ...
  currentArtifact?: RaxArtifactData;
  currentAction: RaxActionData;
  actionId: number;
}
...
export class StreamingMessageParser {
  ...
  parse(messageId: string, input: string) {
    ...
    while (i < input.length) {
      if (input.startsWith(RAX_QUICK_ACTIONS_OPEN, i)) {
        const actionsBlockEnd = input.indexOf(RAX_QUICK_ACTIONS_CLOSE, i);

        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(i + RAX_QUICK_ACTIONS_OPEN.length, actionsBlockEnd);

          // Find all <rax-quick-action ...>label</rax-quick-action> inside
          const quickActionRegex = /<rax-quick-action([^>]*)>([\s\S]*?)<\/rax-quick-action>/g;
          let match;
          const buttons = [];
          ...
          i = actionsBlockEnd + RAX_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as RaxAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as RaxAction,
              });
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__raxQuickAction__"',
    'data-rax-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__raxQuickAction__\" data-rax-quick-action=\"true\">${buttons.join('')}</div>`;
}
