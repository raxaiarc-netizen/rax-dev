...
export function simplifyRaxActions(input: string): string {
  // Using regex to match raxAction tags that have type="file"
  const regex = /(<raxAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/raxAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<raxAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</raxAction>`;
    });

  return `<raxArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</raxArtifact>`;
}
