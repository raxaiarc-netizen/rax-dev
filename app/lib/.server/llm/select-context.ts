...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyRaxActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyRaxActions(content);

      content = content.replace(/<div class=\\"__raxThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`Rax failed to select files`);
  }
