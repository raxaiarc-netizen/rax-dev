...
function sanitizeText(text: string): string {
  let sanitized = text.replace(/<div class=\\"__raxThought__\\">.*?<\/div>/s, '');
  sanitized = sanitized.replace(/<think>.*?<\/think>/s, '');
  sanitized = sanitized.replace(/<raxAction type="file" filePath="package-lock\.json">[\s\S]*?<\/raxAction>/g, '');
...
export async function streamText(props: {
  ...
  // Filter out messages with empty content to prevent LLM API errors
  // Keep messages with parts (like tool invocations) even if content is empty
  processedMessages = processedMessages.filter((message, index) => {
    // Always keep messages with parts (tool invocations, attachments, etc.)
    if (message.parts && message.parts.length > 0) {
      return true;
    }
    
    // For messages without parts, ensure content is not empty
    const hasContent = message.content && message.content.trim().length > 0;
    
    if (!hasContent) {
      logger.warn(`Filtering out message at index ${index} with empty content`, {
        role: message.role,
        hasContent,
  ...
  if (processedMessages.length === 0) {
    throw new Error('No valid messages found after sanitization. All messages had empty content.');
  }
  ...
  if (chatMode === 'build' && contextFiles && contextOptimization) {
    ...
    if (summary) {
      ...
      if (props.messageSliceId && props.messageSliceId > 0) {
        processedMessages = processedMessages.slice(props.messageSliceId);
      } else {
        // Check if we have template initialization messages (user, assistant with template, hidden user)
        // Template messages typically have 3 messages: initial user request, assistant with raxArtifact, and hidden user instructions
        const hasTemplateMessages = processedMessages.length >= 3 && 
          processedMessages.some((msg, idx) => 
            msg.role === 'assistant' && 
            msg.content?.includes('<raxArtifact') &&
            idx < processedMessages.length - 1
          );

        if (hasTemplateMessages) {
          // Keep all template messages - don't slice them
          logger.info('Template messages detected - preserving all messages for context');
        } else {
          // For regular chats, keep only the last message when we have a summary
