...
import * as deploymentIntegration from '~/lib/integrations/deploymentIntegration';
...
export class WorkbenchStore {
  ...
  get raxTerminal() {
    return this.#terminalStore.raxTerminal;
  }
  get alert() {
  ...
  attachRaxTerminal(terminal: ITerminal) {
    this.#terminalStore.attachRaxTerminal(terminal);
    
    // Set up callback to switch to preview when dev server starts
    this.raxTerminal.onDevServerStart = () => {
  ...
  async saveFile(filePath: string) {
    ...
    // Queue deployment update if deployment system is enabled
    if (deploymentIntegration.shouldDeployFile(filePath)) {
      deploymentIntegration.queueFileUpdate(filePath, document.value);
    }
  }
   ...
   * Call this when starting a new project or chat
   */
  initializeProjectDeployment(projectId: string) {
    try {
      deploymentIntegration.initializeDeployment(projectId);
      
      // Create initial deployment with current files
      const files = this.files.get();
      const packageJson = files['package.json'];
      const deps = packageJson && packageJson.type === 'file' && !packageJson.isBinary
        ? deploymentIntegration.extractDependencies(packageJson.content)
        : undefined;
      
      deploymentIntegration.createInitialDeployment(files, deps);
    } catch (error) {
  ...
  addArtifact({ messageId, title, id, type }: ArtifactCallbackData) {
    ...
    this.artifacts.setKey(id, {
      ...
      runner: new ActionRunner(
        ...
        (show: boolean) => {
          this.toggleTerminal(show);
        },
        (view: 'code' | 'preview' | 'diff') => {
