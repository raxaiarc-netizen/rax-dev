...
function makeNonInteractive(command: string): string {
  ...
  const interactivePackages = [
    ...
    { pattern: /pnpm\s+install(?!\s+--)/g, replacement: 'pnpm install' },
...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<raxAction type="shell">${commands.setupCommand}</raxAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<raxAction type="start">${commands.startCommand}</raxAction>
`;
  }
...
<raxArtifact id="project-setup" title="Project Setup">
${commandString}
</raxArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapeRaxArtifactTags(input: string) {
  // Regular expression to match raxArtifact tags and their content
  const regex = /(<raxArtifact[^>]*>)([\s\S]*?)(<\/raxArtifact>)/g;
...
export function escapeRaxAActionTags(input: string) {
  // Regular expression to match raxArtifact tags and their content
  const regex = /(<raxAction[^>]*>)([\s\S]*?)(<\/raxAction>)/g;
...
export function escapeRaxTags(input: string) {
  return escapeRaxArtifactTags(escapeRaxAActionTags(input));
}
