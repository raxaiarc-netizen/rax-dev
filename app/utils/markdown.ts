  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__raxThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__raxArtifact__', '__raxThought__', '__raxQuickAction', '__raxSelectedElement__', '__raxDevPlan__'],

      // ['className', '__raxThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__raxArtifact__', '__raxThought__', '__raxQuickAction'],
    ],
  },
  strip: [],
};
