...
export class RaxShell {
  ...
  async init(webcontainer: WebContainer, terminal: ITerminal) {
    ...
    const { process, commandStream, expoUrlStream } = await this.newRaxShellProcess(webcontainer, terminal);
  ...
  async newRaxShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
    const args: string[] = [];
    const process = await webcontainer.spawn('/bin/jsh', ['--osc', ...args], {
      terminal: {
        cols: terminal.cols ?? 80,
  ...
  async executeCommand(sessionId: string, command: string, abort?: () => void): Promise<ExecutionResult> {
    ...
    // Auto-detect if pnpm install completed successfully and run dev server
    if (resp?.exitCode === 0) {
      const trimmedCommand = command.trim();
      const isPnpmInstall = /^pnpm\s+(install|i)(?:\s|$)/.test(trimmedCommand);
      const isNpmInstall = /^npm\s+(install|i|ci)(?:\s|$)/.test(trimmedCommand);
      
      if (isPnpmInstall || isNpmInstall) {
        console.log('[RaxShell] Install completed successfully, checking for package.json...');
        ...
        try {
          ...
          // Check for dev, start, or preview scripts
          const preferredCommands = ['dev', 'start', 'preview'];
          const availableCommand = preferredCommands.find((cmd) => scripts[cmd]);
          
          if (availableCommand) {
            console.log(`[RaxShell] Found "${availableCommand}" script, auto-running...`);
            
            // Wait a bit to let the terminal settle
            await new Promise(resolve => setTimeout(resolve, 500));
            
            // Auto-run the dev command
            const devCommand = isPnpmInstall ? `pnpm run ${availableCommand}` : `npm run ${availableCommand}`;
            this.terminal.input(devCommand + '\n');
            
            console.log(`[RaxShell] Executed: ${devCommand}`);
            
            // Notify that dev server is starting (switch to preview after delay)
            if (this.onDevServerStart) {
              setTimeout(() => {
                this.onDevServerStart?.();
                console.log('[RaxShell] Switched to preview view');
              }, 3000); // Wait 3 seconds for server to start
            }
          }
        } catch (error) {
          console.log('[RaxShell] Could not auto-run dev server:', error);
        }
      }
    }
...
export function newRaxShellProcess() {
  return new RaxShell();
}
