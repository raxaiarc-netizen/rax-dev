...
async function llmCallAction({ context, request }: ActionFunctionArgs) {
  ...
  if (streamOutput) {
    ...
    try {
      ...
      if (!modelDetails) {
        logger.error(`Model "${model}" not found in available models:`, models.map(m => m.name));
        throw new Error(`Model "${model}" not found. Available models: ${models.slice(0, 5).map(m => m.name).join(', ')}...`);
      }
