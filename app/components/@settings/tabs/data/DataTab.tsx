...
// import { openDatabase } from '~/lib/persistence/db'; // Removed - using cloud storage now
// import { getAllChats, type Chat } from '~/lib/persistence/chats'; // Removed - using cloud storage now
import { DataVisualization } from './DataVisualization';
import { classNames } from '~/utils/classNames';
import { toast } from 'react-toastify';

// Removed - using cloud storage now
// Create a custom hook to connect to the raxHistory database
function useRaxHistoryDB() {
  const [db, setDb] = useState<IDBDatabase | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);

  // Database functionality removed - migrated to cloud storage
...
export function DataTab() {
  // Use our custom hook for the raxHistory database
  const { db, isLoading: dbLoading } = useRaxHistoryDB();
  ...
  // Data operations hook with raxHistory database
  const {
    isExporting,
    isImporting,
    isResetting,
    ...
    customDb: db || undefined, // Pass the raxHistory database, converting null to undefined
    onReloadSettings: () => window.location.reload(),
    onReloadChats: () => {
      // Reload chats after reset - Cloud storage migration
      // TODO: Implement loading chats from cloud storage using ChatAPI
      setAvailableChats([]);
      setChatItems([]);
    },
  ...
  // Load available chats - Disabled (cloud storage migration)
  useEffect(() => {
    // TODO: Implement loading chats from cloud storage
    // Using ChatAPI.listChats() instead of local IndexedDB
    setAvailableChats([]);
    setChatItems([]);
  }, [db]);
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Chats Section */}
      <div>
        <h2 className="text-xl font-semibold mb-4 text-rax-elements-textPrimary">Chats</h2>
        {dbLoading ? (
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center mb-2">
                  ...
                  <CardTitle className="text-lg group-hover:text-rax-elements-item-contentAccent transition-colors">
                    Export All Chats
                  </CardTitle>
                </div>
                ...
                <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="w-full">
                  <Button
                    ...
                    className={classNames(
                      'hover:text-rax-elements-item-contentAccent hover:border-rax-elements-item-backgroundAccent hover:bg-rax-elements-item-backgroundAccent transition-colors w-full justify-center',
      ...
      {/* Settings Section */}
      <div>
        <h2 className="text-xl font-semibold mb-4 text-rax-elements-textPrimary">Settings</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <Card>
            <CardHeader>
              <div className="flex items-center mb-2">
      ...
      {/* API Keys Section */}
      <div>
        <h2 className="text-xl font-semibold mb-4 text-rax-elements-textPrimary">API Keys</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <Card>
            <CardHeader>
              <div className="flex items-center mb-2">
      ...
      {/* Data Visualization */}
      <div>
        <h2 className="text-xl font-semibold mb-4 text-rax-elements-textPrimary">Data Usage</h2>
        <Card>
