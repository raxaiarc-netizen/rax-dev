...
export function RepositoryCard({
    ...
    return (
      <button
        ...
        className={classNames(
          'w-full text-left p-3 rounded-lg border border-rax-elements-borderColor hover:border-rax-elements-borderColorActive hover:bg-rax-elements-background-depth-1 transition-all duration-200',
          className,
        )}
      >
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-medium text-rax-elements-textPrimary">{repository.name}</h4>
            {repository.private && <Lock className="w-3 h-3 text-rax-elements-textTertiary" />}
            {repository.fork && <GitFork className="w-3 h-3 text-rax-elements-textTertiary" />}
            {repository.archived && <Archive className="w-3 h-3 text-rax-elements-textTertiary" />}
          </div>
        ...
        {repository.description && (
          <p className="text-xs text-rax-elements-textSecondary mb-2 line-clamp-2">{repository.description}</p>
        )}

        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-xs text-rax-elements-textTertiary">
            {repository.language && (
              <span className="flex items-center gap-1">
          ...
          <span className="flex items-center gap-1 text-xs text-rax-elements-textTertiary">
  ...
  const interactiveProps = onSelect
    ? {
        onClick: onSelect,
        className: classNames(
          'group cursor-pointer hover:border-rax-elements-borderColorActive dark:hover:border-rax-elements-borderColorActive transition-all duration-200',
          className,
        ),
      }
    : { className };
  ...
  return (
    <Component
      {...interactiveProps}
      className={classNames(
        'block p-4 rounded-lg bg-rax-elements-background-depth-1 dark:bg-rax-elements-background-depth-1 border border-rax-elements-borderColor dark:border-rax-elements-borderColor relative',
    ...
    >
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <GitBranch className="w-4 h-4 text-rax-elements-icon-tertiary" />
            <h5
              className={classNames(
                'text-sm font-medium text-rax-elements-textPrimary',
                onSelect && 'group-hover:text-rax-elements-item-contentAccent transition-colors',
              )}
            >
              {repository.name}
            </h5>
            {repository.fork && (
              <span title="Forked repository">
                <GitFork className="w-3 h-3 text-rax-elements-textTertiary" />
              </span>
            )}
            {repository.archived && (
              <span title="Archived repository">
                <Archive className="w-3 h-3 text-rax-elements-textTertiary" />
              </span>
            )}
          </div>
          <div className="flex items-center gap-3 text-xs text-rax-elements-textSecondary">
            <span className="flex items-center gap-1" title="Stars">
              <Star className="w-3.5 h-3.5 text-rax-elements-icon-warning" />
              {repository.stargazers_count.toLocaleString()}
            </span>
            <span className="flex items-center gap-1" title="Forks">
              <GitFork className="w-3.5 h-3.5 text-rax-elements-icon-info" />
              {repository.forks_count.toLocaleString()}
            </span>
            {showExtendedMetrics && repository.issues_count !== undefined && (
              <span className="flex items-center gap-1" title="Open Issues">
                <Circle className="w-3.5 h-3.5 text-rax-elements-icon-error" />
            ...
            {showExtendedMetrics && repository.pull_requests_count !== undefined && (
              <span className="flex items-center gap-1" title="Pull Requests">
                <GitPullRequest className="w-3.5 h-3.5 text-rax-elements-icon-success" />
        ...
        <div className="space-y-2">
          {repository.description && (
            <p className="text-xs text-rax-elements-textSecondary line-clamp-2">{repository.description}</p>
          )}

          {/* Repository metrics bar */}
          <div className="flex items-center gap-2 text-xs">
            {repository.license && (
              <span className="px-2 py-0.5 rounded-full bg-rax-elements-background-depth-2 text-rax-elements-textTertiary">
            ...
            {repository.fork && (
              <span className="px-2 py-0.5 rounded-full bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-blue-400">
                Fork
              </span>
            )}
          </div>
            ...
            {onSelect && (
              <span
                className={classNames(
                  ...
                  'group-hover:text-rax-elements-item-contentAccent',
                )}
              >
