...
export class GitHubErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="flex flex-col items-center justify-center p-8 text-center space-y-4 bg-rax-elements-background-depth-1 border border-rax-elements-borderColor rounded-lg">
          <div className="w-12 h-12 rounded-full bg-red-50 dark:bg-red-900/20 flex items-center justify-center">
            <AlertTriangle className="w-6 h-6 text-red-500" />
          </div>

          <div>
            <h3 className="text-lg font-medium text-rax-elements-textPrimary mb-2">GitHub Integration Error</h3>
            <p className="text-sm text-rax-elements-textSecondary mb-4 max-w-md">
              Something went wrong while loading GitHub data. This could be due to network issues, API limits, or a
              temporary problem.
            </p>

            {this.state.error && (
              <details className="text-xs text-rax-elements-textTertiary mb-4">
                <summary className="cursor-pointer hover:text-rax-elements-textSecondary">Show error details</summary>
                <pre className="mt-2 p-2 bg-rax-elements-background-depth-2 rounded text-left overflow-auto">
