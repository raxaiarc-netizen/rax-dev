...
function ContextMenuItem({ onSelect, children }: { onSelect?: () => void; children: ReactNode }) {
  return (
    <ContextMenu.Item
      onSelect={onSelect}
      className="flex items-center gap-2 px-2 py-1.5 outline-0 text-sm text-rax-elements-textPrimary cursor-pointer ws-nowrap text-rax-elements-item-contentDefault hover:text-rax-elements-item-contentActive hover:bg-rax-elements-item-backgroundActive rounded-md"
    >
...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    <div
      className="flex items-center w-full px-2 bg-rax-elements-background-depth-4 border border-rax-elements-item-contentAccent py-0.5 text-rax-elements-textPrimary"
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
    >
      <div className="scale-120 shrink-0 i-ph:file-plus text-rax-elements-textTertiary" />
      <input
        ref={inputRef}
        type="text"
        className="ml-2 flex-1 bg-transparent border-none outline-none py-0.5 text-sm text-rax-elements-textPrimary placeholder:text-rax-elements-textTertiary min-w-0"
    ...
    <>
      <ContextMenu.Root>
        <ContextMenu.Trigger>
          <div
            ...
            className={classNames('relative', {
              'bg-rax-elements-background-depth-2 border border-dashed border-rax-elements-item-contentAccent rounded-md':
                isDragging,
            })}
          >
            {children}
        ...
        <ContextMenu.Portal>
          <ContextMenu.Content
            style={{ zIndex: 998 }}
            className="border border-rax-elements-borderColor rounded-md z-context-menu bg-rax-elements-background-depth-1 dark:bg-rax-elements-background-depth-2 data-[state=open]:animate-in animate-duration-100 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-98 w-56"
          >
            <ContextMenu.Group className="p-1 border-b-px border-solid border-rax-elements-borderColor">
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-rax-elements-borderColor">
              {!isFolder ? (
                <>
    ...
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent text-rax-elements-item-contentDefault hover:text-rax-elements-item-contentActive hover:bg-rax-elements-item-backgroundActive':
            !selected,
          'bg-rax-elements-item-backgroundAccent text-rax-elements-item-contentAccent': selected,
        })}
        depth={folder.depth}
...
function File({
  ...
  // Optimize diff calculation with better memoization
  const { additions, deletions } = useMemo(() => {
    ...
    const latestVersion = fileModifications.versions[fileModifications.versions.length - 1];
    
    if (!latestVersion) {
      return { additions: 0, deletions: 0 };
    }
    
    const normalizedCurrent = latestVersion.content.replace(/\r\n/g, '\n');

    // Quick check to avoid expensive diff if content is the same
    if (normalizedOriginal === normalizedCurrent) {
      return { additions: 0, deletions: 0 };
    }

    // Only calculate diff if there are actual changes
  ...
  }, [fileModifications?.originalContent, fileModifications?.versions[fileModifications?.versions.length - 1]?.content]);
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent hover:bg-rax-elements-item-backgroundActive text-rax-elements-item-contentDefault':
            !selected,
          'bg-rax-elements-item-backgroundAccent text-rax-elements-item-contentAccent': selected,
        })}
        depth={depth}
        iconClasses={classNames('i-ph:file-duotone scale-98', {
          'group-hover:text-rax-elements-item-contentActive': !selected,
        })}
        onClick={onClick}
      >
        <div
