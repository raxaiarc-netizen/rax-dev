    ...
    const components = useMemo(() => {
      return {
        div: ({ className, children, node, ...props }) => {
          ...
          if (className?.includes('__raxArtifact__')) {
            ...
            // Render artifact - it will return null if title is empty (no wrapper div will be rendered)
            return <Artifact messageId={messageId} artifactId={artifactId} />;
          }

          if (className?.includes('__raxSelectedElement__')) {
            const messageId = node?.properties.dataMessageId as string;
            ...
            return (
              <div className="bg-rax-elements-background-depth-3 border border-rax-elements-borderColor rounded-lg p-3 my-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-mono bg-rax-elements-background-depth-2 px-2 py-1 rounded text-rax-elements-textTer">
                    {elementData?.tagName}
                  </span>
                  {elementData?.className && (
                    <span className="text-xs text-rax-elements-textSecondary">.{elementData.className}</span>
                  )}
                </div>
                <code className="block text-sm !text-rax-elements-textSecondary !bg-rax-elements-background-depth-2 border border-rax-elements-borderColor p-2 rounded">
          ...
          if (className?.includes('__raxThought__')) {
            return <ThoughtBox title="Thought process">{children}</ThoughtBox>;
          }

          if (className?.includes('__raxDevPlan__')) {
            return <DevelopmentPlanCard>{children}</DevelopmentPlanCard>;
          }

          if (className?.includes('__raxQuickAction__') || dataProps?.dataRaxQuickAction) {
        ...
        button: ({ node, children, ...props }) => {
          ...
          if (
            dataProps?.class?.toString().includes('__raxQuickAction__') ||
            dataProps?.dataRaxQuickAction === 'true'
          ) {
            ...
            return (
              <button
                className="rounded-md justify-center px-3 py-1.5 text-xs bg-rax-elements-item-backgroundAccent text-rax-elements-item-contentAccent opacity-90 hover:opacity-100 flex items-center gap-2 cursor-pointer"
                data-type={type}
 ...
 * const input = "```xml\n<div class='__raxArtifact__'></div>\n```";
 * stripCodeFenceFromArtifact(input);
 * // Returns: "\n<div class='__raxArtifact__'></div>\n"
 *
 * @remarks
 * - Only removes code fences that directly wrap an artifact (marked with __raxArtifact__ class)
 * - Handles code fences with optional language specifications (e.g. ```xml, ```typescript)
 * - Preserves original content if no artifact is found
 * - Safely handles edge cases like empty input or artifacts at start/end of content
 */
export const stripCodeFenceFromArtifact = (content: string) => {
  if (!content || (!content.includes('__raxArtifact__') && !content.includes('__raxDevPlan__'))) {
    return content;
  }

  const lines = content.split('\n');
  const artifactLineIndex = lines.findIndex((line) => line.includes('__raxArtifact__') || line.includes('__raxDevPlan__'));
